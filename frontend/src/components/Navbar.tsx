...
import { Sun, Moon, Menu, Upload, CheckCircle, Loader2, AlertCircle } from 'lucide-react'
import { useSearchStore } from '@/store/useSearchStore'
import { useRef, useState } from 'react'
import { uploadDocument, pollIngestionStatus, type IngestionStatus } from '@/lib/api'

type UploadState = 'idle' | 'uploading' | 'processing' | 'done' | 'error'

export function Navbar() {
  const { theme, setTheme } = useTheme()
  const { sidebarOpen, toggleSidebar, backendOnline } = useSearchStore()
